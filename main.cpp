...
int main(int argc, char **argv) {
    ...
    // Load template image
    cv::Mat targetImage = cv::imread("/Users/utkubelemir/Desktop/AYBU-GraduationProject/lighter.png");
    ...
    // (80, 100, 100 - 125, 255, 255) is color space for all blue colors

    // Find objects in lower color bound;
    cv::inRange(targetImage, cv::Scalar(80, 100, 100), cv::Scalar(100, 255, 255), templateColorLowerBound);

    // Find objects in upper color bound;
    cv::inRange(targetImage, cv::Scalar(100, 100, 100), cv::Scalar(125, 255, 255), templateColorUpperBound);
    ...
    while (true) {
        ...
        // Find objects in lower color bound;
        cv::inRange(realTimeHSV, cv::Scalar(80, 100, 100), cv::Scalar(100, 255, 255), colorLowerBound);

        // Find objects in upper color bound;
        cv::inRange(realTimeHSV, cv::Scalar(100, 100, 100), cv::Scalar(125, 255, 255), colorUpperBound);
        ...
        for (contourIterator = 0; contourIterator < realTimeContours.size(); contourIterator++) {
            cv::Rect spotArea = cv::boundingRect(realTimeContours[contourIterator]);
            if (spotArea.area() < 3500) {
                continue;
            }
            cv::rectangle(realTimeFrame, spotArea, cv::Scalar(226, 0, 247), 5);
        }
        if (detectedObjectVisible > 5 || (HUMomentsResult < 0.3 && HUMomentsResult > -0.3)) {
